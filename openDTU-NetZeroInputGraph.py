...
def update(frame):
	...
	if not flagOnlyPlot and update.ticks % (update_interval / checkInterval) == 0:
		if not flagReachable:
			...
			# IF BATTERY IS CONNECTED AND update.batterybelowthreshold is TRUE: THIS CANNOT HAPPEN 
			# IF BATTERY IS NOT CONNECTED AND update.batterybelowthreshold is TRUE: SKIP ALL OPERATIONS
			#  
			# IF BATTERY IS NOT CONNECTED, AND update.batteryBelowThreshold is FALSE, OPERATE NORMALLY.
			# IF BATTERY IS CONNECTED AND update.batterybelowthreshold is FALSE: OPERATE NORMALLY
			# IF VOLTAGE BELOW THRESHOLD => SET update.batterybelowthreshold to TRUE and set LIMIT TO 0
...
try:
	animation = FuncAnimation(figure, update, blit=False, init_func=init, cache_frame_data=False, interval=500)
	init()
	plt.show()
except:
	log(f'Beende..')
